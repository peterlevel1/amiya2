  ...
  return {
    ...
    onConfirm: async () => {
      try {
        ...
        const data: any = await searchTable.deleteApi(params);

        success('删除成功');

        // 如果删除的是最后一条, 自动向前翻页，最小页数为1
        if (searchTable.tableRef?.current) {
          const tableData = searchTable.tableRef.current.getTableData() ?? [];

          if (tableData.length > 1) {
            searchTable.tableRef.current.refresh();
          } else {
            const apiParams = searchTable.tableRef.current.getApiParams() ?? {};
            if (apiParams.pageNo <= 1) {
              searchTable.tableRef.current.refresh();
            } else {
              const apiParams2 = extend(true, {}, apiParams);
              apiParams2.pageNo -= 1;
              searchTable.tableRef.current.reset(apiParams2);
            }
          }
        }
    ...
    onClick: () => {
      ...
      if (searchTable?.deleteApi) {
        Modal.confirm({
          ...
          onOk: () => {
            let params: Array<string> = selection.map(
              (row: any) => row[searchTable?.rowKey || 'id'],
            );
  ...
  return {
    ...
    onClick: () => {
      searchTable.tableRef.current.addRow({
        [searchTable.rowKey || 'id']: Date.now(),
        editing: true,
      });
    },
    ...props,
