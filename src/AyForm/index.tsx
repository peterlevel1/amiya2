...
): AyFormField | AySearchTableField => {
  return {
    ...field,
    title: '',
    type: 'empty',
...
export const getDefaultValue = (
  fields: Array<AyFormField | AySearchField | AySearchTableField>,
) => {
  let form: AnyKeyProps = {};
  fields.forEach((field: AyFormField | AySearchField | AySearchTableField) => {
    if (
      field.type === FORM_TYPE_CARD ||
      field.type === FORM_TYPE_GROUP ||
      field.type === FORM_TYPE_INPUT_GROUP
    ) {
      form = {
        ...form,
  ...
  for (let key in values) {
    ...
    if (value && field) {
      ...
      let formatRule: string =
        field?.props?.showTime === true ? 'YYYY-MM-DD HH:mm:ss' : 'YYYY-MM-DD';
      if (field.formatRule) {
